...
def refund(params):
    card_num=params['card_num']
    if len(card_num) != 16 or not card_num.isdigit():
        return {'result':None, "error": 'Неверный номер'}
    cvv=params['cvv']
    if len(cvv) != 3 or not cvv.isdigit():
        return {'result':None, "error": 'Неверный CVV'}
        
    price=calculate_price(params)
    return{"result": "success", "message": f"На карту {card_num} вернулись {price} руб", "error": None}